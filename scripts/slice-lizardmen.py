"""Slice lizardman sheet and rebuild full monster atlas."""
...
SRC = Path(
    ...
    r"_ChatGPT_Image_2026__7__25_____04_49_34-0e6043cf-b1d3-407d-a52f-f832e8e2ce87.png"
)
OUT_ROOT = Path(r"I:\Cursor\ExGame\game\assets\textures\monsters")
NAMES = ["lizardman", "black-lizardman", "elder-lizardman"]
TYPE_IDS = {
    "slime": "monster-slime",
    "wolf": "monster-wolf",
    "golem": "monster-golem",
    ...
    "werewolf": "monster-werewolf",
    "red-wolf": "monster-red-wolf",
    "lycanthrope": "monster-lycanthrope",
    "lizardman": "monster-lizardman",
    "black-lizardman": "monster-black-lizardman",
    "elder-lizardman": "monster-elder-lizardman",
}
MAX_SIZES = {
    "lizardman": 96,
    "black-lizardman": 96,
...
def slice_new() -> None:
    ...
    shutil.copy2(SRC, source_dir / "lizardmen_sheet.png")
    ...
    if len(segs) < 3:
        raise SystemExit(f"expected 3 lizardmen, found {segs}")
    segs = sorted(segs, key=lambda s: s[1] - s[0], reverse=True)[:3]
    segs = sorted(segs, key=lambda s: s[0])
    print("lizard segments", segs, "sheet", sheet.size)
