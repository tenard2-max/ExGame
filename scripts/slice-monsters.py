"""Split monster sheet into slime / wolf / golem (character only, no grass)."""
from __future__ import annotations

import json
import shutil
from collections import deque
from pathlib import Path

import numpy as np
from PIL import Image

SRC = next(
    Path(r"C:\Users\lee\.cursor\projects\i-Cursor-ExGame\assets").glob(
        "*25572e56-54c5-40ff-b2e9-a539a0b2fcfc.png"
    )
)
OUT_ROOT = Path(r"I:\Cursor\ExGame\game\assets\textures\monsters")
NAMES = ["slime", "wolf", "golem"]
# 픽셀아트는 과도한 축소+보간이 깨짐의 주원인 → 여유 있게 두고 NEAREST로 맞춤
MAX_SIZES = {"slime": 64, "wolf": 96, "golem": 96}


def find_body_segments(mask: np.ndarray, min_width: int = 80) -> list[tuple[int, int]]:
    col = mask.mean(axis=0) > 0.01
    segs: list[tuple[int, int]] = []
    start: int | None = None
    for i, on in enumerate(col):
        if on and start is None:
            start = i
        if not on and start is not None:
            if i - start >= min_width:
                segs.append((start, i - 1))
            start = None
    if start is not None and len(col) - start >= min_width:
        segs.append((start, len(col) - 1))
    return segs


def remove_black(rgba: np.ndarray, threshold: float = 20.0) -> np.ndarray:
    out = rgba.copy()
    bri = out[:, :, :3].mean(axis=2)
    out[bri < threshold, 3] = 0
    return out


def is_platform_green(r: float, g: float, b: float) -> bool:
    """발판 잔디(밝은 노란-초록). 늑대 회색·골렘 돌·슬라임 본체와 구분."""
    if g < 90:
        return False
    if g <= r + 15:  # 노란기 있는 잔디는 R도 꽤 큼
        return False
    if g <= b + 25:
        return False
    # 슬라임(순녹·하이라이트)보다 잔디는 R 비율이 높음
    return r >= 35 and (r / max(g, 1)) >= 0.28


def remove_bottom_grass_platform(rgba: np.ndarray, aggressive: bool = False) -> np.ndarray:
    """
    하단 가장자리와 연결된 잔디 발판만 flood-fill로 제거합니다.
    aggressive=True 이면 늑대/골렘용으로 초록 판정을 넓힙니다.
    """
    out = rgba.copy()
    h, w = out.shape[:2]
    r = out[:, :, 0].astype(np.float32)
    g = out[:, :, 1].astype(np.float32)
    b = out[:, :, 2].astype(np.float32)
    a = out[:, :, 3]

    def is_grass(rr: float, gg: float, bb: float) -> bool:
        if aggressive:
            # 어두운 올리브 발판까지 포함. 회색/갈색 털(r≈g)은 제외.
            if gg < 28:
                return False
            if gg <= rr * 1.05:
                return False
            if bb > gg * 0.75:
                return False
            return True
        return is_platform_green(rr, gg, bb)

    platform = np.zeros((h, w), dtype=bool)
    y_start = int(h * (0.40 if aggressive else 0.55))
    for y in range(y_start, h):
        for x in range(w):
            if a[y, x] > 0 and is_grass(r[y, x], g[y, x], b[y, x]):
                platform[y, x] = True

    visited = np.zeros((h, w), dtype=bool)
    q: deque[tuple[int, int]] = deque()
    # 하단에서 시드 — 발판이 끊겨 있어도 잡히도록 여유 있게
    seed_from = max(y_start, h - max(12, h // 5))
    for x in range(w):
        for y in range(h - 1, seed_from - 1, -1):
            if platform[y, x] and not visited[y, x]:
                visited[y, x] = True
                q.append((y, x))

    while q:
        y, x = q.popleft()
        out[y, x, 3] = 0
        for ny, nx in ((y - 1, x), (y + 1, x), (y, x - 1), (y, x + 1)):
            if ny < 0 or ny >= h or nx < 0 or nx >= w:
                continue
            if visited[ny, nx] or not platform[ny, nx]:
                continue
            visited[ny, nx] = True
            q.append((ny, nx))

    return out


def remove_small_green_droplets(rgba: np.ndarray, max_area: int = 120) -> np.ndarray:
    """본체와 분리된 작은 초록 방울을 제거합니다."""
    out = rgba.copy()
    h, w = out.shape[:2]
    r = out[:, :, 0].astype(np.float32)
    g = out[:, :, 1].astype(np.float32)
    b = out[:, :, 2].astype(np.float32)
    a = out[:, :, 3]
    green = (a > 0) & (g > 100) & (g > r * 1.2) & (g > b * 1.2)
    visited = np.zeros((h, w), dtype=bool)

    for y in range(h):
        for x in range(w):
            if not green[y, x] or visited[y, x]:
                continue
            q: deque[tuple[int, int]] = deque([(y, x)])
            visited[y, x] = True
            cells: list[tuple[int, int]] = []
            while q:
                cy, cx = q.popleft()
                cells.append((cy, cx))
                for ny, nx in ((cy - 1, cx), (cy + 1, cx), (cy, cx - 1), (cy, cx + 1)):
                    if ny < 0 or ny >= h or nx < 0 or nx >= w:
                        continue
                    if visited[ny, nx] or not green[ny, nx]:
                        continue
                    visited[ny, nx] = True
                    q.append((ny, nx))
            if len(cells) <= max_area:
                for cy, cx in cells:
                    out[cy, cx, 3] = 0
    return out


def trim_alpha(rgba: np.ndarray, pad: int = 2) -> np.ndarray:
    alpha = rgba[:, :, 3] > 8
    if not alpha.any():
        return rgba
    ys, xs = np.where(alpha)
    y0 = max(0, int(ys.min()) - pad)
    y1 = min(rgba.shape[0], int(ys.max()) + pad + 1)
    x0 = max(0, int(xs.min()) - pad)
    x1 = min(rgba.shape[1], int(xs.max()) + pad + 1)
    return rgba[y0:y1, x0:x1]


def fit_max(image: Image.Image, max_side: int) -> Image.Image:
    w, h = image.size
    scale = min(max_side / max(w, h), 1.0)
    if scale >= 0.999:
        return image
    nw = max(1, int(round(w * scale)))
    nh = max(1, int(round(h * scale)))
    # 픽셀아트는 LANCZOS가 흐릿하게 깨지므로 최근접 보간을 씁니다.
    return image.resize((nw, nh), Image.Resampling.NEAREST)


def close_alpha_holes(rgba: np.ndarray, iterations: int = 2) -> np.ndarray:
    """본체 안의 작은 투명 구멍을 메워 아웃라인이 안으로 새지 않게 합니다."""
    out = rgba.copy()
    alpha = out[:, :, 3] > 8
    h, w = alpha.shape
    for _ in range(iterations):
        dilated = alpha.copy()
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                if dx == 0 and dy == 0:
                    continue
                shifted = np.zeros_like(alpha)
                y0 = max(0, dy)
                y1 = min(h, h + dy)
                x0 = max(0, dx)
                x1 = min(w, w + dx)
                sy0 = max(0, -dy)
                sy1 = sy0 + (y1 - y0)
                sx0 = max(0, -dx)
                sx1 = sx0 + (x1 - x0)
                shifted[y0:y1, x0:x1] = alpha[sy0:sy1, sx0:sx1]
                dilated |= shifted
        # erode dilated back roughly by requiring majority — use dilated as fill mask
        # 단순: 불투명 이웃이 있으면 구멍 픽셀을 주변색으로 채움
        fill = dilated & ~alpha
        ys, xs = np.where(fill)
        for y, x in zip(ys, xs):
            samples = []
            for ny in range(max(0, y - 1), min(h, y + 2)):
                for nx in range(max(0, x - 1), min(w, x + 2)):
                    if alpha[ny, nx]:
                        samples.append(out[ny, nx])
            if samples:
                mean = np.mean(samples, axis=0).astype(np.uint8)
                out[y, x] = mean
                out[y, x, 3] = 255
        alpha = out[:, :, 3] > 8
    return out


def add_red_silhouette_outline(
    image: Image.Image,
    thickness: int = 2,
    color: tuple[int, int, int, int] = (220, 35, 35, 255),
) -> Image.Image:
    """불투명 픽셀 실루엣 바깥에 빨간 아웃라인을 깔고 캐릭터를 다시 올립니다."""
    src = Image.fromarray(close_alpha_holes(np.asarray(image.convert("RGBA"))), "RGBA")
    pad = thickness + 1
    canvas = Image.new(
        "RGBA",
        (src.width + pad * 2, src.height + pad * 2),
        (0, 0, 0, 0),
    )
    canvas.paste(src, (pad, pad), src)
    arr = np.asarray(canvas).copy()
    alpha = arr[:, :, 3] > 8
    h, w = alpha.shape
    outline = np.zeros((h, w), dtype=bool)
    for dy in range(-thickness, thickness + 1):
        for dx in range(-thickness, thickness + 1):
            if dx == 0 and dy == 0:
                continue
            if dx * dx + dy * dy > thickness * thickness + 1:
                continue
            shifted = np.zeros_like(alpha)
            y0 = max(0, dy)
            y1 = min(h, h + dy)
            x0 = max(0, dx)
            x1 = min(w, w + dx)
            sy0 = max(0, -dy)
            sy1 = sy0 + (y1 - y0)
            sx0 = max(0, -dx)
            sx1 = sx0 + (x1 - x0)
            shifted[y0:y1, x0:x1] = alpha[sy0:sy1, sx0:sx1]
            outline |= shifted
    outline &= ~alpha
    arr[outline, 0] = color[0]
    arr[outline, 1] = color[1]
    arr[outline, 2] = color[2]
    arr[outline, 3] = color[3]
    out = Image.fromarray(arr, "RGBA")
    out.paste(src, (pad, pad), src)
    return out


def main() -> None:
    if OUT_ROOT.exists():
        shutil.rmtree(OUT_ROOT)
    OUT_ROOT.mkdir(parents=True)
    source_dir = OUT_ROOT / "_source"
    source_dir.mkdir()
    shutil.copy2(SRC, source_dir / "monsters_sheet.png")

    sheet = Image.open(SRC).convert("RGBA")
    arr = np.asarray(sheet)
    bri = arr[:, :, :3].mean(axis=2)
    mask = bri > 18
    segs = find_body_segments(mask, min_width=80)
    if len(segs) < 3:
        raise SystemExit(f"expected 3 monsters, found {segs}")
    segs = sorted(segs, key=lambda s: s[1] - s[0], reverse=True)[:3]
    segs = sorted(segs, key=lambda s: s[0])
    print("segments", segs)

    frames_meta: dict[str, dict] = {}
    atlas_images: list[tuple[str, Image.Image]] = []

    for name, (x0, x1) in zip(NAMES, segs):
        pad = 10
        crop = sheet.crop((max(0, x0 - pad), 0, min(sheet.width, x1 + pad + 1), sheet.height))
        rgba = remove_black(np.asarray(crop))
        rgba = trim_alpha(rgba)
        rgba = remove_bottom_grass_platform(
            rgba,
            aggressive=(name != "slime"),
        )
        if name == "slime":
            rgba = remove_small_green_droplets(rgba, max_area=120)
        rgba = trim_alpha(rgba)
        img = Image.fromarray(rgba, "RGBA")
        img = fit_max(img, MAX_SIZES[name])
        img = add_red_silhouette_outline(img, thickness=2)
        out_path = OUT_ROOT / f"{name}.png"
        img.save(out_path)
        frames_meta[name] = {
            "file": f"{name}.png",
            "width": img.width,
            "height": img.height,
            "typeId": f"monster-{name}",
        }
        atlas_images.append((name, img))
        print(f"{name}: {img.size} -> {out_path}")

    gap = 4
    atlas_w = sum(im.width for _, im in atlas_images) + gap * (len(atlas_images) - 1)
    atlas_h = max(im.height for _, im in atlas_images)
    atlas = Image.new("RGBA", (atlas_w, atlas_h), (0, 0, 0, 0))
    atlas_frames = []
    x = 0
    for name, im in atlas_images:
        y = atlas_h - im.height
        atlas.paste(im, (x, y), im)
        atlas_frames.append(
            {
                "name": name,
                "typeId": f"monster-{name}",
                "x": x,
                "y": y,
                "w": im.width,
                "h": im.height,
            }
        )
        x += im.width + gap

    atlas.save(OUT_ROOT / "atlas.png")
    (OUT_ROOT / "atlas.json").write_text(
        json.dumps(
            {
                "width": atlas_w,
                "height": atlas_h,
                "frames": atlas_frames,
            },
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
    )
    (OUT_ROOT / "manifest.json").write_text(
        json.dumps({"monsters": frames_meta}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print("atlas", atlas.size)


if __name__ == "__main__":
    main()
