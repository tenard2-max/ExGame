"""Slice werewolf sheet and rebuild full monster atlas (keep existing sprites)."""
...
SRC = Path(
    ...
    r"_ChatGPT_Image_2026__7__25_____04_20_14-6a26e005-5df7-4672-af3b-2fe472e42d59.png"
)
OUT_ROOT = Path(r"I:\Cursor\ExGame\game\assets\textures\monsters")
NAMES = ["werewolf", "red-wolf", "lycanthrope"]
TYPE_IDS = {
    "slime": "monster-slime",
    "wolf": "monster-wolf",
    "golem": "monster-golem",
...
def slice_new() -> None:
    ...
    shutil.copy2(SRC, source_dir / "werewolves_sheet.png")
    ...
    if len(segs) < 3:
        raise SystemExit(f"expected 3 werewolves, found {segs}")
    segs = sorted(segs, key=lambda s: s[1] - s[0], reverse=True)[:3]
    segs = sorted(segs, key=lambda s: s[0])
    print("werewolf segments", segs, "sheet", sheet.size)
